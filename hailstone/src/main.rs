...
fn main(){
    let mut num = getIntFromStdin(); //mut because they may be modified
    let mut steps = 0;
    loop {
        if num <= 0 {	//panic b/c this doesn't make sense
	    panic!("What do you want from me?!?!")
	}
        if num%2 == 0 {	//just the hailstone calculation part
            num = num / 2;
            steps += 1;
	...
	if num == 1 {	//if it is 1, then we are done so print the status update 
...
fn getIntFromStdin() -> u64 { //u64 so they can test large numbers
